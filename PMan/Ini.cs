...
class Ini {
    ...
    internal static string GetConfig(string Key, string Name, string CfgFile, bool Required = true) {
        if (!Required && !File.Exists(CfgFile))
            return string.Empty;

        string[] Lines = File.ReadAllLines(CfgFile, Encoding.UTF8);
        return GetConfig(Key, Name, Lines, Required);
    }
    internal static string GetConfig(string Key, string Name, string[] Lines, bool Required = true) {        
    ...
    internal static ConfigStatus GetConfigStatus(string Key, string Name, string CfgFile) {
        ...
        return GetConfigStatus(Key, Name, Lines);
    }

    internal static ConfigStatus GetConfigStatus(string Key, string Name, string[] Lines) {        
        bool KeyFound = false;
        string AtualKey = string.Empty;
        foreach (string Line in Lines) {
            if (Line.StartsWith("[") && Line.EndsWith("]"))
